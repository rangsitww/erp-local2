...
def make_delivery_note(source_name, target_doc=None, skip_item_mapping=False):
	...
	def update_item(source, target, source_parent):
		...
		#--------------------------------------------------------------
		xcv = source.conversion_factor
		if not xcv:
			xcv = 1
		
		sub_sales_qty = (source.qty2 * xcv) + source.xqty
		bal_sub_sales_qty = sub_sales_qty -source.delivered_qty 
		...
		target.qty = flt(deli_qty_full)
		target.qty_sub = flt(deli_qty_sub )
