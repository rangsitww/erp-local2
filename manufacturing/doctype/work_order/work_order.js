...
erpnext.work_order = {
	set_custom_buttons: function(frm) {
		...
		if (doc.docstatus === 1) {
			...
			if (show_start_btn) {
				if (((flt(doc.material_transferred_for_manufacturing)+flt(doc.material_transferred_for_manufacturing_sub)) < (flt(doc.qty)+flt(doc.sub_qty)))
			...
			if(!frm.doc.skip_transfer){
				...
				if (((flt(doc.produced_qty) + flt(doc.produced_sub_qty)) < (flt(doc.material_transferred_for_manufacturing)+flt(doc.material_transferred_for_manufacturing_sub)))
				&& frm.doc.status != 'Stopped') {
					...
					if((doc.material_transferred_for_manufacturing + doc.material_transferred_for_manufacturing_sub)   >= (doc.qty + doc.sub_qty)) {
	...
	get_sub_max_transferable_qty: (frm, purpose) => {
		let sub_max = 0;
		if (frm.doc.skip_transfer) {
			sub_max = flt(frm.doc.qty) - flt(frm.doc.produced_qty);
		} else {

			sub_max = flt(frm.doc.sub_qty);
		}
		return flt(sub_max, precision('sub_qty'));
	},

	show_prompt_for_qty_input: function(frm, purpose) {
		let max = this.get_max_transferable_qty(frm, purpose);
		let sub_max = this.get_sub_max_transferable_qty(frm, purpose);
		...
		return new Promise((resolve, reject) => {
			...
			},
			{
				...
				if (data.qty > max) {
					frappe.msgprint(__('Quantity xxmust not be more than {0}', [max]));
					reject();
				}
