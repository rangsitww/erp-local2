...
class WorkOrder(Document):
	...
	def update_status2(self, status=None):
		'''Update status of work order if unknown'''
		self.db_set("status", status)
	...
	def get_status(self, status=None):
		...
		if self.docstatus==0:
			...
			if status != 'Stopped':
				...
				stock_entries2 = frappe._dict(frappe.db.sql("""select purpose, sum(fg_completed_sub_qty)
					from `tabStock Entry` where work_order=%s and docstatus=1
					group by purpose""", self.name))

				status = "Not Started"
				if stock_entries:
					#frappe.throw('rangsit')
					status = "In Process"
					produced_qty = stock_entries.get("Manufacture")
					produced_sub_qty = stock_entries2.get("Manufacture")

					if (flt(produced_qty)+flt(produced_sub_qty)) >= (flt(self.qty)+flt(self.sub_qty)):
						status = "Completed"
		else:
			status = 'Cancelled'
	...
	def update_work_order_qty(self):
		...
		for purpose, fieldname in (("Manufacture", "produced_qty"),
			...
			qty2 = flt(frappe.db.sql("""select sum(fg_completed_sub_qty)
				from `tabStock Entry` where work_order=%s and docstatus=1
				and purpose=%s""", (self.name, purpose))[0][0])
			...
			if (fieldname == 'produced_sub_qty'):
				#self.db_set('produced_sub_qty', qty2)
				self.db_set(fieldname, qty2)
			else:
				self.db_set(fieldname, qty)

			self.db_set('material_transferred_for_manufacturing_sub', qty2)
	...
	def validate_qty(self):
		if not (self.qty+self.sub_qty) > 0:
	...
	def update_transaferred_qty_for_required_items(self):
		...
		for d in self.required_items:
			...
			transferred_sub_qty = frappe.db.sql('''select sum(sub_qty)
				from `tabStock Entry` entry, `tabStock Entry Detail` detail
				where
					entry.work_order = %(name)s
					and entry.purpose = "Material Transfer for Manufacture"
			...
			d.db_set('transferred_sub_qty', flt(transferred_sub_qty), update_modified = False)
...
@frappe.whitelist()
def make_stock_entry(work_order_id, purpose, qty=None, sub_qty=None):
	...
	stock_entry.fg_completed_sub_qty = sub_qty
...
@frappe.whitelist()
def closed_wo(work_order, status):
	""" Called from client side on Stop/Unstop event"""
	...
	pro_order = frappe.get_doc("Work Order", work_order)
	pro_order.update_status2(status)
	frappe.msgprint(_("Work Order has been {0}").format(status))
