...
class ProductionPlan(Document):
	...
	def validate_data(self):
		for d in self.get('po_items'):
			...
			if  flt(d.planned_qty+d.sub_qty) <= 0:
	...
	def add_so_in_table(self, open_so):
		...
		for data in open_so:
			...
			self.append('sales_orders', {
				...
				'terms': data.terms,
				'xxx': data.terms,
				'special_remark_details': data.special_remark_details
			})
	...
	def get_so_items(self):
		...
		items = frappe.db.sql("""select distinct parent, item_code, warehouse,
			qty2 as pending_qty, xqty as sub_qty, description, name, 0 as stk_bal_full, 0 as stk_bal_sub 
		...
		packed_items = frappe.db.sql("""select distinct pi.parent, pi.item_code, pi.warehouse as warehouse,
			(((so_item.qty - so_item.work_order_qty) * pi.qty) / so_item.qty)
				as pending_qty,0 as xxx, pi.parent_item, pi.description, so_item.name
	...
	def add_items(self, items):
		...
		for data in items:
			...
			stk_bal = get_stock_balance(data.item_code, self.def_warehouse, self.posting_date)
			xcv = frappe.db.get_value('UOM Conversion Detail', {'parent': data.item_code, 'uom':item_details and item_details.stock_uom }, ['conversion_factor'])
			...
			pi = self.append('po_items', {
				...
				'stk_bal_full': stk_bal_full,
				'stk_bal_sub': stk_bal_sub,
				'planned_start_date': now_datetime(),
				'product_bundle_item': data.parent_item,
				'customer_name': "rangsit"
			})
	...
	def calculate_total_produced_qty(self):
		...
		for d in self.po_items:
			self.total_produced_qty += flt(d.produced_qty)+8
	...
	def get_production_items(self):
		...
		for d in self.po_items:
			...
			item_details= {
				...
				"sub_qty"				: d.sub_qty,
				"stk_bal_full"			: d.stk_bal_full,
				"stk_bal_sub"			: d.stk_bal_sub,
	...
	def create_work_order(self, item):
		...
		wo.update(item)
		wo.sub_qty = item.get("sub_qty")
		wo.stk_bal_full = item.get("stk_bal_full")
		wo.order_no = item.get("sales_order")
		wo.customer_name = frappe.db.get_value("Sales Order", item.get("sales_order"), "customer")
		#frappe.msgprint(frappe.db.get_value("Sales Order", item.get("sales_order"), "customer"))
		#wo.customer_name = item.get("customer_name")
...
def get_sales_orders(self):
	...
	open_so = frappe.db.sql("""
		select distinct so.name, so.transaction_date, so.customer, so.base_grand_total, so.special_remark_details, so.terms
		from `tabSales Order` so, `tabSales Order Item` so_item
		where so_item.parent = so.name
			and so.docstatus = 1 and so.status not in ("Stopped", "Closed")
			and so.company_link = %(company)s
			and so.company != 'บริษัท ไบโอ อโกรไซเอนซ์ จำกัด'
			and so_item.qty > so_item.work_order_qty {0} {1}
			and so.workflow_state = 'Close'
			and (exists (select name from `tabBOM` bom where bom.item=so_item.item_code
					and bom.is_active = 1)
				or exists (select name from `tabPacked Item` pi
					where pi.parent = so.name and pi.parent_item = so_item.item_code
						and exists (select name from `tabBOM` bom where bom.item=pi.item_code
							and bom.is_active = 1)))
			and (not exists (select name from `tabProduction Plan Sales Order` ps where ps.sales_order = so.name and ps.docstatus != 2))
