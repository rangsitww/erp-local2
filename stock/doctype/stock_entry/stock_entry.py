...
class StockEntry(StockController):
	...
	def set_transfer_qty(self):
		for item in self.get("items"):
			if not flt(item.qty) and not flt(item.sub_qty):
				frappe.throw(_("Row {0}: Qty is mandatory").format(item.idx))
			if not flt(item.conversion_factor):
				frappe.throw(_("Row {0}: UOM Conversion Factor is mandatory").format(item.idx))
			
			x=item.sub_qty
			#frappe.throw(str(item.sub_qty))
			item.transfer_qty = flt(flt(item.qty) * flt(item.conversion_factor)+x,
	...
	def validate_work_order(self):
		if self.purpose in ("Manufacture", "Material Transfer for Manufacture", "Material Consumption for Manufacture"):
			...
			if (self.purpose=="Manufacture" or self.purpose=="Material Consumption for Manufacture") \
					and self.work_order:
				if (flt(self.fg_completed_qty)+flt(self.fg_completed_sub_qty)) <= 0:
	...
	def validate_finished_goods(self):
		...
		for d in self.get('items'):
			#frappe.throw(_("Quantityxxx in row {0} {1}"). \
			#		format(d.uom,d.transfer_qty))
			...
			#if (self.purpose != "Send to Subcontractor" and d.bom_no
			#	and d.item_code == production_item):
				#frappe.throw(_("Quantity in row {0} ({1}) must be same as manufactured quantity {2}"). \
				#	format(d.idx, d.transfer_qty, self.fg_completed_qty))
			#	frappe.throw(_("Quantityxxxyyy in row {0} {1} {2} conversion = {3} total = {4}"). \
			#		format(self.fg_completed_qty,d.transfer_qty,d.uom,d.conversion_factor,total_transfer))

			total_transfer = (self.fg_completed_qty * d.conversion_factor)

			#if (self.purpose != "Send to Subcontractor" and d.bom_no
			#	and flt(d.transfer_qty) > flt(total_transfer) and d.item_code == production_item):
			#	frappe.throw(_("Quantity in row {0} ({1}) must be same as manufactured quantity {2}"). \
			#		format(d.idx, d.qty, self.fg_completed_qty))
	...
	def update_work_order(self):
		...
		if self.work_order:
			...
			if (self.fg_completed_qty + self.fg_completed_sub_qty) > 0:
	...
	def add_to_stock_entry_detail(self, item_dict, bom_no=None):
		...
		for d in item_dict:
			...
			if se_child.item_code == production_item:
				se_child.sub_qty = flt(self.fg_completed_sub_qty)
