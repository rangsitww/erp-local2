...
frappe.ui.form.on("Stock Reconciliation", {
	...
	set_valuation_rate_and_qty: function(frm, cdt, cdn) {
		...
		//frappe.msgprint('ass');
		//frappe.msgprint(frm.doc.from_warehouse);
		//frappe.msgprint(d.item_code);
		//var x = frappe.db.get_value('Item','AAA12ABG','stock_uom'  );
		//var y = frappe.db.get_value("User", "acc1@egmail.com", "last_login");
		//frappe.msgprint(frm.doc.company);

		
		frappe.model.set_value(cdt, cdn, "warehouse", frm.doc.from_warehouse);
				...
				callback: function(r) {
					...
					//frappe.model.set_value(cdt, cdn, "qty", r.message.qty);
						...
						if(!r.message.qty) {
							...
							frappe.model.set_value(cdt, cdn, "current_qty", 0);
							frappe.model.set_value(cdt, cdn, "current_valuation_rate", 0);
							frappe.model.set_value(cdt, cdn, "current_amount", 0);
					...
					if (!r.exe){
						...
						frappe.call({
							...
							callback: function(rx){
								if(r.message){
									//frappe.msgprint(r.message.item_name);
									x_item_name = rx.message.item_name;
									x_stock_uom = rx.message.stock_uom;
									frappe.model.set_value(cdt, cdn, "item_name", x_item_name);
									frappe.model.set_value(cdt, cdn, "stock_uom", x_stock_uom);
