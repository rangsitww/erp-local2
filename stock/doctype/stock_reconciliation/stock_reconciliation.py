...
class StockReconciliation(StockController):
	...
	def get_sle_for_items(self, row, serial_nos=None):
		...
		xcv = frappe.db.get_value('UOM Conversion Detail', {'parent': row.item_code, 'conversion_factor': ['>', 1]}, ['conversion_factor'])

		if not xcv:
			xcv = 1

		data.qty_after_transaction = flt(row.qty * xcv) + row.sub_qty
